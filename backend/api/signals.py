    ...
    def _generate_sync(ot_id_inner, date_inner):
        try:
            ...
            if export_data_grouped or monthly_data_grouped:
                ...
                logger.info(f"Background thread Excel generation completed for OT {ot_id_inner} (date: {date_inner})")
            else:
                logger.warning(f"No data to export for OT {ot_id_inner} (date: {date_inner})")
        except Exception as sync_err:
            logger.error(f"Background thread Excel generation failed for OT {ot_id_inner}: {sync_err}", exc_info=True)
...
def notify_leave_event_participants(event, is_update=False):
    ...
    if agent and agent.emp_id:
        try:
            agent_user = ExternalUser.objects.get(worker_id__iexact=agent.emp_id, is_active=True)
            ...
            logger.info(f"Notified agent {agent.name} about leave coverage for {employee_name}")
        except ExternalUser.DoesNotExist:
            logger.debug(f"No ExternalUser found for agent {agent.emp_id}")
        except Exception as e:
    ...
    try:
        ...
        logger.info(f"Notified {len(notifs_to_create)} PTB admins about leave for {employee_name}")
    except Exception as e:
        logger.error(f"Error notifying PTB admins about leave: {e}")
...
def notify_purchase_request_status_change(purchase_request, old_status, new_status):
    ...
    if owner_employee and owner_employee.emp_id:
        try:
            recipient = ExternalUser.objects.get(worker_id__iexact=owner_employee.emp_id, is_active=True)
        except ExternalUser.DoesNotExist:
            pass
    ...
    if not recipient and purchase_request.owner:
        ...
        try:
            recipient = ExternalUser.objects.filter(
                username__iexact=owner_str, is_active=True
            ).first()
        except Exception:
            pass
    ...
    if not recipient:
        logger.debug(f"Could not find recipient for purchase request {purchase_request.id}")
        return
    ...
    logger.info(f"Notified {recipient.username} about purchase request status change to {new_status}")
    return notification
...
def notify_ptb_admins_new_purchase_request(purchase_request):
    ...
    try:
        ...
        logger.info(f"Notified {len(ptb_admins)} PTB admins about new purchase request")
        return created if notifs_to_create else []
    except Exception as e:
        logger.error(f"Error notifying PTB admins about purchase request: {e}")
        return []
    ...
    if action == "post_add" and pk_set:
        try:
            ...
            if notifications_to_create:
                ...
                logger.info(f"Bulk created {len(created_notifications)} notifications for event assignment")
...
def invalidate_employee_cache(sender, instance, created, **kwargs):
    ...
    try:
        ...
        logger.info(f"Employee {action}: {instance.name} (ID: {instance.id})")
        ...
        logger.debug(f"Invalidated employee cache for {instance.name}")
    except Exception as e:
...
def invalidate_employee_cache_on_delete(sender, instance, **kwargs):
    ...
    """
    try:
        logger.info(f"Employee deleted: {instance.name} (ID: {instance.id})")
        ...
        logger.error(f"Error invalidating employee cache on delete: {str(e)}")
...
def invalidate_project_cache(sender, instance, created, **kwargs):
    ...
    try:
        action = "created" if created else "updated"
        logger.info(f"Project {action}: {instance.name} (ID: {instance.id})")
        ...
        logger.debug(f"Invalidated project cache for {instance.name}")
    except Exception as e:
        logger.error(f"Error invalidating project cache: {str(e)}")
...
def invalidate_project_cache_on_delete(sender, instance, **kwargs):
    ...
    """
    try:
        logger.info(f"Project deleted: {instance.name} (ID: {instance.id})")
        ...
        logger.error(f"Error invalidating project cache on delete: {str(e)}")
...
def invalidate_overtime_cache(sender, instance, created, **kwargs):
    ...
    try:
        action = "created" if created else "updated"
        logger.info(f"OvertimeRequest {action}: ID {instance.id}")
        ...
        logger.error(f"Error invalidating overtime cache: {str(e)}")
    ...
    if instance.request_date:
        try:
            ...
            logger.info(
                f"Queued async Excel generation for OvertimeRequest {instance.id} "
                f"(date: {instance.request_date})"
            )
        except Exception as e:
            logger.warning(
                f"Failed to queue async Excel generation for OT {instance.id}: {e}. "
                "Falling back to background thread pool."
            )
...
def invalidate_overtime_cache_on_delete(sender, instance, **kwargs):
    ...
    """
    try:
        logger.info(f"OvertimeRequest deleted: ID {instance.id}")
        ...
        logger.error(f"Error invalidating overtime cache on delete: {str(e)}")
...
def invalidate_calendar_cache(sender, instance, created, **kwargs):
    ...
    try:
        action = "created" if created else "updated"
        logger.info(f"CalendarEvent {action}: ID {instance.id}")
        ...
        logger.error(f"Error invalidating calendar cache: {str(e)}")
...
def invalidate_calendar_cache_on_delete(sender, instance, **kwargs):
    ...
    """
    try:
        logger.info(f"CalendarEvent deleted: ID {instance.id}")
        ...
        logger.error(f"Error invalidating calendar cache on delete: {str(e)}")
...
def handle_purchase_request_save(sender, instance, created, **kwargs):
    ...
    try:
        if created:
            ...
            logger.info(f"Purchase request created: ID {instance.id}")
        else:
            # Check for status change - need to compare with database
            # Since we don't have the old value easily, we'll handle this in the view
            pass
    except Exception as e:
        logger.error(f"Error handling purchase request save: {str(e)}")
