...
let dropzoneInstance: { destroy: () => void } | null = null

onMounted(() => {
	Dropzone.autoDiscover = false

	const instance = new Dropzone(`#${dropzoneId}`, {
		url: props.uploadUrl,
		thumbnailWidth: 150,
		maxFilesize: 0.5,
	...
	instance.on('addedfile', (file: File) => {
		console.log('A file has been added', file)
	})

	instance.on('success', (file: File, response: unknown) => {
		console.log('File successfully uploaded', file, response)
	})

	instance.on('error', (file: File, errorMessage: string | Error) => {
		console.error('An error occurred during upload', file, errorMessage)
	})
