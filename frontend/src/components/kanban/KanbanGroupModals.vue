    ...
    <div v-if="showGroupListModal"
        ...
        <div role="dialog" aria-modal="true" aria-labelledby="kanban-groups-list-title"
            ...
            <div class="p-6 pb-4 border-b border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        <button @click="$emit('update:group-modal-tab', 'create')"
                            class="px-3 py-1.5 text-sm bg-brand-50 dark:bg-brand-900/30 text-brand-700 dark:text-brand-300 rounded-lg hover:bg-brand-100 dark:hover:bg-brand-900/50 transition disabled:opacity-50 flex items-center gap-1.5">
                        ...
                        <button @click="$emit('close-group-list-modal')"
                ...
                <div class="flex gap-2 mt-4">
                    <button @click="$emit('update:group-modal-tab', 'all')" :class="[
                    ...
                    <button @click="$emit('update:group-modal-tab', 'create')" :class="[
                        'px-4 py-2 rounded-lg text-sm font-medium transition',
                    ...
                    <button v-if="editingGroup" @click="$emit('update:group-modal-tab', 'edit')" :class="[
                        'px-4 py-2 rounded-lg text-sm font-medium transition',
                        groupModalTab === 'edit'
                            ? 'bg-brand-600 text-white'
                            : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'
            ...
            <div class="flex-1 overflow-y-auto p-6">
                <div v-if="groupActionError" class="mb-4 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700 dark:border-rose-900/40 dark:bg-rose-900/20 dark:text-rose-200">
                    {{ groupActionError }}
                </div>

                <div v-if="groupModalTab === 'all'" class="space-y-3">
                ...
                <form v-if="groupModalTab === 'create'" @submit.prevent="$emit('save-group')" class="space-y-4">
                    <div>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                        <button type="button" @click="$emit('update:group-modal-tab', 'all')"
                ...
                <div v-if="groupModalTab === 'edit' && editingGroup" class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ t('kanban.groupNameRequired') }}</label>
...
import { FolderIcon, HourglassIcon, PencilIcon, PlusIcon, RefreshIcon, TrashIcon, XIcon } from '@/icons'
...
defineEmits<{
	...
	'cancel-edit-group': []
	'save-edit-group': []
    'update:group-modal-tab': [value: 'all' | 'create' | 'edit']
	'update:groupMemberSearch': [value: string]
}>()

const props = defineProps<{
    showGroupListModal: boolean
    groupModalTab: 'all' | 'create' | 'edit'
