...
watch(
	...
	() => {
		...
		if (props.leave) {
			selectedDates.value =
				props.initialDates && props.initialDates.length > 0
					? sortDateStrings([...props.initialDates])
					: [props.leave.date]
			form.value = {
				notes: props.leave.notes || '',
			}
			selectedEmployee.value = props.employees.find((employee) => employee.id === props.leave!.employee) || null
			if (selectedEmployee.value) {
				searchQuery.value = ''
			}
			selectedAgents.value = Array.isArray(props.leave.agents)
				? props.leave.agents.map((agent) => ({ ...agent }))
				: []
		} else {
