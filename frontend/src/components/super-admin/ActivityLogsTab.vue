        ...
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="border-b border-gray-200 bg-gray-50 px-6 py-4 dark:border-gray-800 dark:bg-gray-900/50">
                <div class="grid grid-cols-1 gap-4 md:grid-cols-5">
                    <div>
                        <label
                            class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Employee</label>
                        <select v-model="activityFilters.user_id" @change="resetPageAndLoad"
                            ...
                            <option v-for="user in userOptions" :key="user.id" :value="user.id">
                                {{ `${user.first_name} ${user.last_name}`.trim() || user.username }} ({{ user.worker_id
                                || user.username }})
                            </option>
                        </select>
                    </div>
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Action
                            Type</label>
                    ...
                    <div>
                        <label
                            class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Resource</label>
                        <input v-model="activityFilters.resource" @input="debounceLoadLogs" type="text"
                            placeholder="e.g. employees, assets..."
                            class="block w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 shadow-theme-xs transition focus:border-brand-500 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white" />
                    </div>
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Date
                            Range</label>
                        <div class="relative">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                    <thead>
                        <tr class="border-b border-gray-200 bg-gray-50 dark:border-gray-800 dark:bg-gray-900/50">
                            <th
                                class="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                                Timestamp</th>
                            <th
                                class="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                                User</th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                        <tr v-if="activityLoading">
                            <td colspan="6" class="px-5 py-8 text-center text-sm text-gray-500 dark:text-gray-400">
                                <div class="flex flex-col items-center gap-2">
                                    <svg class="h-8 w-8 animate-spin text-brand-500" xmlns="http://www.w3.org/2000/svg"
                        ...
                        <tr v-else v-for="log in validLogs" :key="log.id"
                            ...
                            <td class="whitespace-nowrap px-5 py-4 text-sm text-gray-900 dark:text-gray-100">
                                {{ log.resource || '-' }}
                                <span v-if="log.resource_id" class="text-gray-500 dark:text-gray-400">#{{
            ...
            <!-- Pagination -->
            <div v-if="!activityLoading && totalCount > 0"
...
const validLogs = computed(() =>
	activityLogs.value
		...
		.map((l) => ({
			...l,
			_detailsJson: l.details && Object.keys(l.details).length > 0 ? JSON.stringify(l.details) : '',
		})),
)
