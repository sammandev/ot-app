		...
		<div class="space-y-6">
			<div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
				<div>
					...
					<span
						class="rounded-md bg-brand-100 px-2 py-1 text-xs font-medium text-brand-700 dark:bg-brand-900/30 dark:text-brand-200">{{
							t('otSummary.monthCalculation') }}</span>
				</div>
			</div>
			...
			<template v-else>
					...
					<div class="grid gap-4 lg:grid-cols-12">
						...
						<template v-if="dateSelectionType === 'custom'">
							<div class="lg:col-span-6 space-y-2">
								<label class="text-sm font-medium text-gray-700 dark:text-gray-300">{{
									t('otSummary.customDateRange') }}</label>
								<div class="relative">
				...
				<div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
					<div v-for="card in summaryCards" :key="card.title"
						...
						<p class="mt-1 text-xs text-gray-400 dark:text-gray-500">{{ t('otSummary.vsPreviousPeriod') }}
						</p>
					</div>
				</div>
					...
					<div
						...
						<div class="flex items-center justify-between pb-3">
							<h3 class="text-base font-semibold text-gray-900 dark:text-white">{{
								t('otSummary.overtimeByProject') }}
							</h3>
							<span class="text-xs text-gray-500 dark:text-gray-400">{{ t('otSummary.topProjects')
								}}</span>
						</div>
				...
				<div class="grid gap-4 lg:grid-cols-2">
					<div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
								...
								<thead
									...
									<tr>
										<th @click="toggleEmployeeSort('name')"
											@keydown.enter="toggleEmployeeSort('name')" tabindex="0"
											:aria-sort="employeeSortBy === 'name' ? (employeeSortOrder === 'asc' ? 'ascending' : 'descending') : 'none'"
											class="px-3 py-2 cursor-pointer select-none">
											{{ t('otSummary.employee') }}<span class="text-gray-400">{{
												getSortIcon(employeeSortBy,
													'name',
													employeeSortOrder) }}</span></th>
										<th @click="toggleEmployeeSort('hours')"
											@keydown.enter="toggleEmployeeSort('hours')" tabindex="0"
										...
										<th @click="toggleEmployeeSort('requests')"
											@keydown.enter="toggleEmployeeSort('requests')" tabindex="0"
											:aria-sort="employeeSortBy === 'requests' ? (employeeSortOrder === 'asc' ? 'ascending' : 'descending') : 'none'"
											class="px-3 py-2 cursor-pointer select-none">
											{{ t('otSummary.requests') }}<span class="text-gray-400">{{
												getSortIcon(employeeSortBy,
													'requests',
													employeeSortOrder) }}</span></th>
										<th @click="toggleEmployeeSort('avgHours')"
											@keydown.enter="toggleEmployeeSort('avgHours')" tabindex="0"
											:aria-sort="employeeSortBy === 'avgHours' ? (employeeSortOrder === 'asc' ? 'ascending' : 'descending') : 'none'"
											class="px-3 py-2 cursor-pointer select-none">{{ t('otSummary.avgHour')
											}}<span class="text-gray-400">{{ getSortIcon(employeeSortBy, 'avgHours',
												employeeSortOrder) }}</span></th>
									</tr>
								</thead>
						...
						<div class="mt-3 flex items-center justify-between text-sm text-gray-600 dark:text-gray-400">
							<span>{{ t('otSummary.showingRange', {
								start: employeeRangeStart, end: employeeRangeEnd,
								total:
								employeeStats.length }) }}</span>
							<div class="flex items-center gap-2">
								...
								<span>{{ t('otSummary.pageOf', { page: employeePage, total: employeeTotalPages })
									}}</span>
					...
					<div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
								...
								<thead
									...
									<tr>
										<th @click="toggleProjectSort('name')"
											@keydown.enter="toggleProjectSort('name')" tabindex="0"
											:aria-sort="projectSortBy === 'name' ? (projectSortOrder === 'asc' ? 'ascending' : 'descending') : 'none'"
											class="px-3 py-2 cursor-pointer select-none">
											{{ t('otSummary.project') }}<span class="text-gray-400">{{
												getSortIcon(projectSortBy,
													'name',
													projectSortOrder) }}</span>
										</th>
										<th @click="toggleProjectSort('hours')"
											@keydown.enter="toggleProjectSort('hours')" tabindex="0"
										...
										<th @click="toggleProjectSort('requests')"
											@keydown.enter="toggleProjectSort('requests')" tabindex="0"
										...
										<th @click="toggleProjectSort('avgHours')"
											@keydown.enter="toggleProjectSort('avgHours')" tabindex="0"
											:aria-sort="projectSortBy === 'avgHours' ? (projectSortOrder === 'asc' ? 'ascending' : 'descending') : 'none'"
											class="px-3 py-2 cursor-pointer select-none">{{ t('otSummary.avgHour')
											}}<span class="text-gray-400">{{ getSortIcon(projectSortBy, 'avgHours',
												projectSortOrder)
											}}</span></th>
									</tr>
								</thead>
						...
						<div class="mt-3 flex items-center justify-between text-sm text-gray-600 dark:text-gray-400">
							<span>{{ t('otSummary.showingRange', {
								start: projectRangeStart, end: projectRangeEnd,
								total:
								projectStats.length }) }}</span>
							<div class="flex items-center gap-2">
								<button @click="setProjectPage(projectPage - 1)" :disabled="projectPage === 1"
									class="h-8 rounded-lg border border-gray-300 bg-white px-3 text-sm font-medium text-gray-800 shadow-theme-xs transition hover:bg-gray-50 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 disabled:cursor-not-allowed disabled:opacity-50 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:hover:bg-white/5">
									{{ t('common.prev') }}
								</button>
								<span>{{ t('otSummary.pageOf', { page: projectPage, total: projectTotalPages })
									}}</span>
...
import { FLATPICKR_READY_DELAY_MS } from '@/constants/ui'
